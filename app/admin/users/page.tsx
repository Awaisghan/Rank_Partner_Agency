...
import { Plus, Search, Check, X, Shield, User, Edit, Users, AlertTriangle, Trash2 } from "lucide-react";
...
export default function AdminUsersPage() {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      mutate();
      showSuccess(`${deleteTarget.name} has been deleted.`);
    } catch (err: any) {
      setDeleteTarget(null);
      setFormError(err.message);
    }
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          ...
                          className="inline-flex items-center justify-center w-7 h-7 rounded text-slate-400 hover:text-[#6d28d9] hover:bg-violet-50 transition-colors cursor-pointer"
                          title="Edit User"
                        >
                        ...
                        {user.role !== "ADMIN" && (
                          <button
                            ...
                            className="inline-flex items-center justify-center w-7 h-7 rounded text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors cursor-pointer"
                            title="Delete User"
                          >
